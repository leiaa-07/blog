...
![how to focus ur eyes](/static/images/blogs/stereogram-2.jpg)
...
1. https://youtu.be/v8O8Em_RPNg?si=fJp7DMK6t3SLGgjt
2. https://youtu.be/-okxLz1UauA?si=w-W4DECbDGS55Ht-
...
okay so since these 2 imgs are the same, the identical imgs will just fuse tgt and nothin special will happen womp womp. BUT the magic happens when u displace some pixels from one of the imgs by a tiny amount. so for example, i could take a square of pixels, and shift each of those pixels by one pixel to the right. (then the missing space on the left can just be filled in with other random pixels.) oh and ofc u only shift the pixels for one of the imgs la, the other img is untouched. OKAY SO THENNNNN, when u try to view it again, the 2 imgs will fuse right, and the pixels which are the same will just "overlap" kinda, but for the pixels that were shifted, ur left eye and right eye are gonna perceive slightly shifted imgs SO THENN when the img fuses, ur depth perception will basically kick in so it'll show up as 3d AAAA ITS SO COOL AND I HOPE THAT EXPLANATION MADE SENSE HAHAHAJDJFSSHHF
