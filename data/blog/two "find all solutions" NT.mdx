...
i COMPLETELY missed that T-T. but if u're not blind like me, u could then rewrite the equation as 
<center>
...
</center>but notice that $k \geq 1$ implies $-1$ is a QR mod $p$, so $p \equiv 1 \pmod{4}$ by fermat's christmas theorem, but this is clearly a contradiction by the first condition in the problem statement! so we must have $k=v_2(n+1)=0 \implies$ $n$ is even.
